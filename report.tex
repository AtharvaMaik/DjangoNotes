...
\begin{enumerate}
    \item \textbf{No-Reload Interaction}: Every core action (CRUD) is asynchronous.
    \item \textbf{Visual Hierarchy}: Prominent task statistics provide immediate utility.
    \item \textbf{Responsive Feedback}: Transitions keep the user connected to the data lifecycle.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Database: SQLite}: Used via the Django ORM to provide a robust storage engine.
\end{itemize}

\section{Scalability and Efficiency}
While currently utilizing an SQLite backend, the system is designed with scalability in mind. The Django ORM allows for a ``plug-and-play'' transition to databases like PostgreSQL. The client-side search filtering is optimized to handle hundreds of notes with minimal latency.
...
\begin{enumerate}
    ...
    \item \textbf{The Feedback}: The server returns a success signal, and the UI updates instantly.
\end{enumerate}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{validation_error.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{edit_modal.png}
    \caption{The Edit Modal centered on the viewport.}
\end{figure}

\newpage
\section{Conclusion}
The Minimalist Notes Application successfully fulfills all technical and design requirements. By bridging the gap between Python's server-side strengths and JavaScript's frontend agility, we have produced a modern, sleek, and highly functional application.

\begin{center}
    \vspace{2cm}
    \textbf{END OF PROJECT REPORT}\\
    \small Atharva Maikhuri (235816206) | Harshita Gaurav (235816036)\\
    April 14, 2026
\end{center}
